...
static int					check_obj_raw_data(char *raw_data, int data_len)
{
	if (data_len == 2 && raw_data[0] == 'g')
		return (1);
    return (data_len > 2 ? dispatch_by_header(raw_data, data_len) : 0);
}

static int					check_mtl_raw_data(char *raw_data, int data_len)
{
	if (data_len)
		return (0);
	return (1);
        ...
        short parsing_type)
{
	...
	return ((parsing_type == P_OBJ) ?
		check_obj_raw_data(raw_data, data_len) :
		check_mtl_raw_data(raw_data, data_len));
}